...
impl Generator {
    ...
    pub fn generate(&self, output_dir: String, file_name: String, _skip_errors: bool) {
        ...
        let mut dimension_data_existence_types: Vec<String> = Vec::new();
            ...
            if let Some(union) = process_cube(cube) {
                for prefix in &config.prefixes {
                    if prefix.name == cube.name {
                        write_main_types(
                            TypeVariant::Dimension,
                            &prefix.prefix,
                            &union,
                            &mut output,
                            &mut all_dimension_types,
                        );
                        ...
                        if config.enable_count_types {
                            write_count_types(
                                ...
                                &union,
                                &mut output,
                                &mut measure_count_types,
                            );
                        }

                        if config.enable_check_existence_fields {
                            write_existence_types(
                                &prefix.prefix,
                                &union,
                                &mut output,
                                &mut dimension_data_existence_types,
                            );
                        }
...
fn write_main_types(
    ...
    match variant {
        TypeVariant::Dimension => {
            let dimension_type_name = format!("{}Dimensions", prefix);
            output.push('\n');
            output.push_str(&format!(
                "export type {} = {}",
            ...
            all.extend(dimension_type_name.split(" | ").map(String::from));
        }
        TypeVariant::Measure => {
            let measure_type_name = format!("{}Measures", prefix);
            output.push('\n');
            output.push_str(&format!(
                "export type {} = {}",
            ...
            all.extend(measure_type_name.split(" | ").map(String::from));
        }
    }
}

fn write_existence_types(
    prefix: &str,
    union: &CubeUnion,
    output: &mut String,
    dimension_data_existence_types: &mut Vec<String>,
) {
    ...
    if !dimension_data_existence_types.is_empty() {
        output.push('\n');
        output.push_str(&format!(
            "export type {} = {}",
            dimension_data_existence,
            dimension_data_existence_types.join(" | ")
        ));
        output.push('\n');
    }
}

fn write_count_types(
    prefix: &str,
    union: &CubeUnion,
    output: &mut String,
    measure_count_types: &mut Vec<String>,
) {
    let measure_counts = format!("{}CountMeasures", prefix);
...
fn extract_name(full_name: &'_ str) -> Cow<'_, str> {
    full_name
        .rsplit_once('.')
